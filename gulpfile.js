...
export function fonts(cb) {
  return src(["src/www/**/*.ttf", "src/www/**/*.woff", "src/www/**/*.woff2"], {
    encoding: false,
  }).pipe(dest(DEST));
}

export function other(cb) {
  src(["src/www/site.webmanifest", "src/www/CNAME"]).pipe(dest(DEST));

  // Binary files go here
  src(["src/www/favicon.ico", "src/www/wedding-brochure.pdf"], {
    encoding: false,
  }).pipe(dest(DEST));

  src(["src/www/.well-known/assetlinks.json"]).pipe(
    dest(DEST + "/.well-known/"),
  );
...
export function scripts() {
  return src("src/www/**/*.js").pipe(dest(DEST));
}

export function imagesSvg() {
  return src("src/www/**/*.svg").pipe(dest(DEST));
}

export function images() {
  return (
    src(["src/www/**/*.png", "src/www/**/*.jpg", "src/www/**/*.webp"], {
      encoding: false,
    })
...
export function styles() {
  return src("src/www/**/*.css").pipe(dest(DEST));
}
...
export function deps(cb) {
  ...
  src("node_modules/@splidejs/splide/dist/css/splide.min.css").pipe(
    dest(DEST + "/css/"),
  );
...
const watchFiles = series(parallel(deps, www, invite), function (cb) {
  bs.init({
    open: false,
    server: {
      baseDir: "www",
